<template>
  <div class="wave_container">
    <svg :width="svg.width" :height="svg.height" id="svg">
      ...
      <g id="wave">
        <path
          fill="rgba(54,89,119,.8)"
          :d="`M ${svg.gap} ${svg.height / 2} C ${svg.width / 2 + 33.633} ${svg.height / 2 - 14.88} ${svg.width / 2 + .54} ${svg.height / 2 + 16.327} ${svg.width - svg.gap} ${svg.height / 2} A ${svg.height / 2 - svg.gap} ${svg.height / 2 - svg.gap} 0 0 0 ${svg.gap} ${svg.width / 2} Z`"
        >
          <animate
            dur="3s"
...
M${svg.gap} ${svg.height / 2} C${svg.width / 2 - 10} ${svg.height / 2 + 20},${svg.width / 2 - 8} ${svg.height / 2 - 20}, ${svg.width - svg.gap} ${svg.height / 2} A${svg.height / 2 - svg.gap} ${svg.height / 2 - svg.gap} 0 0 0 ${svg.gap} ${svg.height / 2} Z`"
        ...
        <path
          :style="{fill: type==='normal'?'#8ee7b4':' #e6d97d'}"
          :d="`M ${svg.gap} ${svg.height / 2 - 10} C ${svg.width / 2 + 33.633} ${svg.height / 2 - 14.88} ${svg.width / 2 + .54} ${svg.height / 2 + 6.327} ${svg.width - svg.gap} ${svg.height / 2} A ${svg.height / 2 - svg.gap} ${svg.height / 2 - svg.gap} 0 0 1 ${svg.gap} ${svg.width / 2} Z`"
        ></path>
        <path
          :style="{fill:type==='normal'?'url(#color)':'url(#color1)'}"
          :d="`M ${svg.gap} ${svg.height / 2} C ${svg.width / 2 + 33.633} ${svg.height / 2 - 14.88} ${svg.width / 2 + .54} ${svg.height / 2 + 16.327} ${svg.width - svg.gap} ${svg.height / 2} A ${svg.height / 2 - svg.gap} ${svg.height / 2 - svg.gap} 0 0 1 ${svg.gap} ${svg.width / 2} Z`"
        >
          <animate
            dur="3s"
            repeatCount="indefinite"
...
M${svg.gap} ${svg.height / 2} C${svg.width / 2 - 10} ${svg.height / 2 + 20},${svg.width / 2 - 8} ${svg.height / 2 - 20}, ${svg.width - svg.gap} ${svg.height / 2} A${svg.height / 2 - svg.gap} ${svg.height / 2 - svg.gap} 0 0 1 ${svg.gap} ${svg.height / 2} Z`"
      ...
      <path
        ...
        fill="none"
        class="ring"
        :style="{stroke: type==='normal'?'#7df681':'#e99720'}"
        stroke-width="3"
      ></path>
      <circle
        ref="circle"
        r="4 "
        stroke-width="3"
        :style="{fill: type==='normal'?'#7df681':'#e99720'}"
        class="percentage-pie-svg"
      ></circle>
      <image
        :x="`${svg.width / 2 - 40}`"
        :y="`${svg.height / 2 - 60}`"
        xlink:href="/static/img/icon/top.png"
        width="15px"
        height="22px"
        v-if="svg.up"
      ></image>
      <text
        ...
        v-if="svg.up"
        font-size="22px"
        :transform="`translate(${svg.width / 2 - 10},${svg.height / 2 - 40})`"
      >{{svg.up}}</text>
      <image
        :x="`${svg.width / 2 - 40}`"
        :y="`${svg.height / 2 + 20}`"
        xlink:href="/static/img/icon/btm.png"
        width="15px"
        height="22px"
        v-if="svg.down"
      ></image>
      ...
      <text
        class="liquidFillGaugeText"
        font-size="48px"
        :transform="`translate(${svg.width / 2 - 30},${svg.height / 2 + 10})`"
        v-if="svg.percent"
      >{{svg.percent+'%'}}</text>
    </svg>
  </div>
...
export default {
  ...
  mounted() {
    ...
    const desc = `M ${r} ${r} m${-x1},${-y1} a${r},${r} 0,0,0 ${x1 -
      r},${y1} a${r},${r} 0,0,0 ${r},${r} a${r},${r} 0 0 0 ${r},${-r} a${r},${r} 0 0 0 ${-r +
      x1},${-y1}`;
